...
def chk_yf_nifty():
    ...
    if df is not None and len(df) > 0:
        try:
            ...
            return True, "NIFTY: close unavailable"
...
def chk_yf_bnk():
    ...
    if df is not None and len(df) > 0:
        try:
            ...
            return True, "BANKNIFTY: close unavailable"
...
def chk_yf_sensex():
    ...
    if df is not None and len(df) > 0:
        try:
            ...
            return True, "SENSEX: close unavailable"
    return False, "No data"
...
def chk_yf_vix():
    ...
    if df is not None and len(df) > 0:
        try:
            ...
            return True, "India VIX: close unavailable"
    return False, "No data"
...
def _market_open_now() -> bool:
    """True only during NSE cash-market hours on a weekday. Outside this window no
    source serves a LIVE option chain (NSE-direct is also IP-blocked here and the
    bot uses the Angel fallback in-hours), so a 404 after close is EXPECTED, not a
    fault — used to stop the option-chain checks crying wolf after market close."""
...
def chk_nse_nifty_oc():
    ...
    # Try Angel fallback when NSE endpoint is unavailable
    try:
        from option_chain_fetcher import NSEOptionChainFetcher
        f = NSEOptionChainFetcher('NIFTY')
        ang = f._fetch_from_angel()
        if ang:
            return True, "Angel fallback OK"
    except Exception:
        pass
    if not _market_open_now():
        return True, "market closed — OC not live (expected)"
...
def chk_nse_bnk_oc():
    ...
    try:
        from option_chain_fetcher import NSEOptionChainFetcher
        f = NSEOptionChainFetcher('BANKNIFTY')
        ang = f._fetch_from_angel()
        if ang:
            return True, "Angel fallback OK"
    except Exception:
...
def chk_nse_finn_oc():
    ...
    try:
        from option_chain_fetcher import NSEOptionChainFetcher
        f = NSEOptionChainFetcher('FINNIFTY')
        ang = f._fetch_from_angel()
        if ang:
            return True, "Angel fallback OK"
    except Exception:
...
def chk_nse_ban():
    ...
    if r.status_code == 200:
        return True, "NSE API OK"
    try:
        from pathlib import Path
        import pandas as pd
        mc = Path("MasterContract_NFO.csv")
        if mc.exists():
            df = pd.read_csv(mc, usecols=lambda c: c in {"symbol", "lotsize", "exch_seg"})
            return True, f"local master fallback OK ({len(df):,} rows)"
    except Exception:
        pass
    return False, f"HTTP {r.status_code}"
...
def chk_nse_participant():
    ...
    try:
        ...
        if data:
            return True, "module/cache fallback OK"
    except Exception:
        pass
    return False, f"HTTP {r.status_code}"
...
def chk_telegram():
    ...
    except (req_exc.ConnectTimeout, req_exc.ReadTimeout):
        return False, "Network timeout to api.telegram.org; check internet/firewall/DNS"
    except req_exc.ConnectionError:
        return False, "Network unreachable to api.telegram.org; check internet/firewall/DNS"
    if r.status_code == 200:
...
def chk_gdrive():
    ...
    if len(present) >= 2:
        return True, f"{len(present)}/3 credential files present"
    remote = os.getenv("GDRIVE_REMOTE", "gdrive")
    folder = os.getenv("GDRIVE_FOLDER", "trading_robot")
    try:
        result = subprocess.run(
            ["rclone", "lsd", f"{remote}:{folder}", "--max-depth", "1"],
            text=True,
        ...
        if result.returncode == 0:
            return True, f"rclone remote OK ({remote}:{folder})"
    except Exception:
        pass
    return False, f"{len(present)}/3 files present and rclone remote unavailable"
...
def chk_news():
    ...
    if not news_key:
        for cache_name in ("news_sentiment_cache.json", "news_nlp_cache.json"):
            cache = Path(cache_name)
            if not cache.exists():
                continue
            try:
                payload = json.loads(cache.read_text(encoding="utf-8", errors="replace"))
                data = payload.get("data", payload) if isinstance(payload, dict) else {}
                headlines = data.get("total_headlines", 0) if isinstance(data, dict) else 0
                avg = data.get("avg_score", data.get("weighted_score", 0.0)) if isinstance(data, dict) else 0.0
                return True, f"cached fallback active ({headlines} headlines, avg={float(avg):+.2f})"
            except Exception:
                return True, f"cached fallback active ({cache_name})"
        return False, "NEWS_API_KEY missing and no news cache fallback found"
