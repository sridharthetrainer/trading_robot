...
VIX_CSV      = Path(os.getenv("VIX_HISTORY_CSV", "vix_history.csv"))
...
def capture_vix() -> Dict[str, Any]:
    """Fetch + append today's India VIX to vix_history.csv (date, vix).

    VIX is fetched live all over the code but was never persisted as a series, so
    it could not be used as a dated regime feature for analysis. This records it
    daily via the working Angel-backed fetch (NSE-direct is IP-blocked here), so
    market_context_by_date() can JOIN VIX regime into the ML/analysis feature
    matrix and its edge can be MEASURED (not assumed). No-op if the fetch fails.
    """
    try:
        from data_source_resilience import get_india_vix
        vix = float(get_india_vix() or 0)
        if vix <= 0:
            return {"saved": False, "reason": "VIX fetch returned 0"}
        today = date.today().isoformat()
        rows: List[Dict[str, str]] = []
        if VIX_CSV.exists():
            try:
                with open(VIX_CSV, newline="") as f:
                    rows = [r for r in csv.DictReader(f) if r.get("date") != today]
            except Exception:
                rows = []
        if rows:  # cap to the most recent KEEP_DAYS distinct dates
            keep = sorted({r["date"] for r in rows})[-KEEP_DAYS:]
            rows = [r for r in rows if r["date"] in keep]
        with open(VIX_CSV, "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=["date", "vix"])
            w.writeheader()
            for r in rows:
                w.writerow({"date": r.get("date", ""), "vix": r.get("vix", "")})
            w.writerow({"date": today, "vix": round(vix, 2)})
        return {"saved": True, "vix": round(vix, 2), "date": today}
    except Exception as exc:
        logger.debug("capture_vix: %s", exc)
        return {"saved": False, "error": str(exc)}
...
def run_eod_capture() -> Dict[str, Any]:
    ...
    logger.info("EOD market capture: FII/DII=%s | sectors=%s | participant_oi=%s | vix=%s | nse=%s",
                fii, sec, poi, vix, {"ok": nse.get("ok"), "ok_count": nse.get("ok_count")})
    return {"fii_dii": fii, "sectors": sec, "participant_oi": poi, "vix": vix, "nse_data_hub": nse}
...
def market_context_by_date() -> Dict[str, Dict[str, float]]:
    ...
    # India VIX regime (vix_history.csv): level, 1-day change, band 0..3
    try:
        if VIX_CSV.exists():
            vrows: List = []
            with open(VIX_CSV, newline="") as f:
                for row in csv.DictReader(f):
                    try:
                        ...
                        if v > 0:
                            vrows.append((str(row["date"])[:10], v))
            ...
            for d, v in vrows:
                ...
                rec["india_vix"] = v
                rec["vix_chg"]   = round(v - prev, 2) if prev is not None else 0.0
                rec["vix_band"]  = 0 if v < 12 else 1 if v < 16 else 2 if v < 22 else 3
                prev = v
    except Exception as exc:
        logger.debug("market_context_by_date VIX: %s", exc)
