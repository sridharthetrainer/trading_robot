...
def test_record_and_load_jsonl() -> bool:
...
def test_limit_recent_rows() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        path = str(Path(tmp) / "journal.jsonl")
        for idx in range(5):
            record_option_decision(
        ...
        return len(rows) == 2 and rows[0]["decision"] == "d3" and rows[1]["decision"] == "d4"


def test_label_option_decision_by_trade_id() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        path = str(Path(tmp) / "journal.jsonl")
        record_option_decision(
            strategy="hero_zero",
...
def test_label_shadow_decisions_by_trade_id() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        path = str(Path(tmp) / "journal.jsonl")
        record_option_decision(
...
def test_auto_shadow_candidates_for_selected() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        path = str(Path(tmp) / "journal.jsonl")
        record_option_decision(
            strategy="pivot_scalping",
...
def test_repair_missing_shadow_candidates() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        return result.get("updated") == 1 and len(rows[0].get("strikes", [])) >= 5
