...
class OptionChainSummary:
    ...
    most_active_call_strikes: List[Dict[str, Any]]
    most_active_put_strikes: List[Dict[str, Any]]
    activity_sentiment: Dict[str, Any]
...
class OptionChainIntelligence:
    ...
    def analyze(
        ...
        activity_sentiment = self._activity_sentiment(df_window, float(spot_price))
        most_active_call_strikes = activity_sentiment.get("top_call_strikes", [])
        most_active_put_strikes = activity_sentiment.get("top_put_strikes", [])
        ...
        bullish_score, bearish_score, net_bias = self._compute_bias_scores(
            ...
            activity_sentiment=activity_sentiment,
        )
        ...
        return OptionChainSummary(
            ...
            most_active_call_strikes=most_active_call_strikes,
            most_active_put_strikes=most_active_put_strikes,
            activity_sentiment=activity_sentiment,
    ...
    def build_trade_signal(
        ...
        reason_parts: List[str] = [
            ...
            f"activity={summary.activity_sentiment.get('sentiment', 'NEUTRAL')}",
        ...
        if signal == "BUY_CALL":
            ...
            if summary.activity_sentiment.get("sentiment") == "BULLISH":
                reason_parts.append("active OI/volume bullish")
        else:
            ...
            if summary.activity_sentiment.get("sentiment") == "BEARISH":
                reason_parts.append("active OI/volume bearish")
        ...
        return {
            ...
            "activity_sentiment": summary.activity_sentiment,
            "most_active_strikes": {
                "calls": summary.most_active_call_strikes,
                "puts": summary.most_active_put_strikes,
            },
        }
    ...
    def _activity_sentiment(self, df: pd.DataFrame, spot_price: float) -> Dict[str, Any]:
        """
        Rank the most traded/high-OI strikes and convert them into sentiment.

        Interpretation:
        - Put OI / put OI buildup below spot is support and bullish.
        - Call OI / call OI buildup above spot is resistance and bearish.
        - Fresh OI change and traded volume get more weight than old OI.
        """
        if df.empty:
            return {
                "sentiment": "NEUTRAL",
        ...
        work = df.copy()
        oi_max = max(
            float(work["CE_openInterest"].max() or 0),
            float(work["PE_openInterest"].max() or 0),
            1.0,
        )
        chg_max = max(
            float(work["CE_changeinOpenInterest"].clip(lower=0).max() or 0),
            float(work["PE_changeinOpenInterest"].clip(lower=0).max() or 0),
            1.0,
        )
        vol_max = max(
            float(work["CE_totalTradedVolume"].max() or 0),
            float(work["PE_totalTradedVolume"].max() or 0),
            1.0,
        )

        work["CE_activity_score"] = (
            0.45 * (work["CE_openInterest"] / oi_max)
            + 0.35 * (work["CE_changeinOpenInterest"].clip(lower=0) / chg_max)
            + 0.20 * (work["CE_totalTradedVolume"] / vol_max)
        )
        work["PE_activity_score"] = (
            0.45 * (work["PE_openInterest"] / oi_max)
            + 0.35 * (work["PE_changeinOpenInterest"].clip(lower=0) / chg_max)
            + 0.20 * (work["PE_totalTradedVolume"] / vol_max)
        )

        def _top(side: str, score_col: str) -> List[Dict[str, Any]]:
            oi_col = f"{side}_openInterest"
            chg_col = f"{side}_changeinOpenInterest"
            vol_col = f"{side}_totalTradedVolume"
            out: List[Dict[str, Any]] = []
            for _, row in work.sort_values(score_col, ascending=False).head(5).iterrows():
                strike = float(row["strikePrice"])
                out.append({
                    "strike": strike,
                    "distance_pct": round((strike - spot_price) / max(spot_price, 1.0) * 100.0, 3),
                    "score": round(float(row[score_col]), 4),
                    "oi": float(row[oi_col]),
                    "change_oi": float(row[chg_col]),
                    "volume": float(row[vol_col]),
                    "zone": "ABOVE_SPOT" if strike > spot_price else "BELOW_SPOT" if strike < spot_price else "ATM",
                })
            return out

        top_calls = _top("CE", "CE_activity_score")
        top_puts = _top("PE", "PE_activity_score")

        bullish_score = 0.0
        bearish_score = 0.0
        for item in top_puts[:3]:
            weight = float(item["score"])
            if item["strike"] <= spot_price:
                bullish_score += weight * 1.25
            else:
                bearish_score += weight * 0.55
        for item in top_calls[:3]:
            weight = float(item["score"])
            if item["strike"] >= spot_price:
                bearish_score += weight * 1.25
            else:
    ...
    def _compute_bias_scores(
        ...
        gamma_support: Optional[float],
        gamma_resistance: Optional[float],
        activity_sentiment: Optional[Dict[str, Any]] = None,
        ...
        activity_sentiment = activity_sentiment or {}
        activity_bias = str(activity_sentiment.get("sentiment", "NEUTRAL"))
        activity_strength = min(abs(float(activity_sentiment.get("imbalance", 0.0) or 0.0)), 2.0)
        if activity_bias == "BULLISH":
            bullish_score += min(1.5, 0.75 + activity_strength * 0.35)
        elif activity_bias == "BEARISH":
            bearish_score += min(1.5, 0.75 + activity_strength * 0.35)
    ...
    def _compute_trade_quality(
        ...
        score = 0.0
        activity_bias = str((activity_sentiment or {}).get("sentiment", "NEUTRAL"))
        activity_strength = min(abs(float((activity_sentiment or {}).get("imbalance", 0.0) or 0.0)), 1.5)
        ...
        if summary_bias == "BULLISH":
            ...
            if call_unwinding_count > 0:
                score += 0.5
            if activity_bias == "BULLISH":
                score += 0.5 + min(activity_strength, 1.0) * 0.25
            elif activity_bias == "BEARISH":
                score -= 0.75
            ...
            if put_unwinding_count > 0:
                score += 0.5
            if activity_bias == "BEARISH":
                score += 0.5 + min(activity_strength, 1.0) * 0.25
            elif activity_bias == "BULLISH":
                score -= 0.75
