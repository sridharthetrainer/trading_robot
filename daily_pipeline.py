...
Steps (in order):
  ...
  4. post_market_ml        — retrain ML filters on today's outcomes
  5. walk_forward_backtest — OOS validation (skipped unless --run-wf flag set;
...
def run_pipeline(
    ...
    # ── Step 4: Post-market ML training ─────────────────────────────────────
    if not dry_run:
        def _step_ml():
    ...
    # ── Step 5: Walk-forward backtest (optional, slow) ───────────────────────
