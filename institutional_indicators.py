...
def detect_market_structure(
    ...
    """
    Detect Break of Structure (BOS), Change of Character (CHOCH), and the
    latest HH/HL/LH/LL swing labels.
    ...
    Returns:
        ...
        labels:          latest and previous high/low swing labels
    ...
    try:
        ...
        if len(swing_highs) < 2 or len(swing_lows) < 2:
            return {"structure": "RANGING", "last_bos": None,
                    "choch_detected": False, "choch_direction": None,
                    "structure_direction": "NEUTRAL",
                    "structure_score": 0.0, "structure_label": "",
                    "labels": {},
        ...
        sh0 = swing_highs[-3][1] if len(swing_highs) >= 3 else None
        sl0 = swing_lows[-3][1] if len(swing_lows) >= 3 else None
        ...
        high_label = "HH" if hh_pattern else "LH" if lh_pattern else "EH"
        low_label = "HL" if hl_pattern else "LL" if ll_pattern else "EL"
        prev_high_label = (
            "HH" if sh0 is not None and sh1 > sh0
            else "LH" if sh0 is not None and sh1 < sh0
            else ""
        )
        prev_low_label = (
            "HL" if sl0 is not None and sl1 > sl0
            else "LL" if sl0 is not None and sl1 < sl0
            else ""
        )
        ...
        prev_uptrend = prev_high_label == "HH" and prev_low_label == "HL"
        prev_downtrend = prev_high_label == "LH" and prev_low_label == "LL"
        choch_direction = None
        if prev_uptrend and (low_label == "LL" or last_bos == "BEARISH"):
            choch_direction = "BEARISH"
        elif prev_downtrend and (high_label == "HH" or last_bos == "BULLISH"):
            choch_direction = "BULLISH"
        elif high_label == "LH" and low_label == "HL":
            choch_direction = "BEARISH"
        elif high_label == "HH" and low_label == "LL":
            choch_direction = "BULLISH"
        choch_detected = bool(choch_direction)
        ...
        if choch_direction == "BULLISH": score += 0.5
        if choch_direction == "BEARISH": score -= 0.5

        if choch_direction == "BULLISH" or last_bos == "BULLISH" or structure == "UPTREND":
            structure_direction = "BULLISH"
        elif choch_direction == "BEARISH" or last_bos == "BEARISH" or structure == "DOWNTREND":
        ...
        return {
            ...
            "labels": {
                "high": high_label,
                "low": low_label,
                "prev_high": prev_high_label,
                "prev_low": prev_low_label,
            },
        ...
        return {"structure": "RANGING", "last_bos": None,
                "choch_detected": False, "choch_direction": None,
                "structure_direction": "NEUTRAL", "structure_label": "",
                "labels": {}, "structure_score": 0.0,
