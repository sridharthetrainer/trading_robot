...
def test_autotune_neutral_until_min_samples() -> bool:
...
def test_autotune_rewards_winning_feature() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        journal = str(Path(tmp) / "journal.jsonl")
        out = str(Path(tmp) / "autotune.json")
        for _ in range(8):
        ...
        return weight > 1.0 and scored["multiplier"] > 1.0


def test_autotune_learns_from_shadow_candidates() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        journal = str(Path(tmp) / "journal.jsonl")
        out = str(Path(tmp) / "autotune.json")
        for _ in range(10):
