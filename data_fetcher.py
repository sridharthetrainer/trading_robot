...
class DataFetcher:
    ...
    def get_market_data(
        ...
        if self.angel and hasattr(self.angel, "get_historical_data"):
            try:
                ...
                if df is not None and not df.empty:
                    df = self._clean(df)
                    self._cache_market_data(cache_key, symbol, interval, df, source="angel")
                    try:
        ...
        if not _yf_disabled:
            try:
                ...
                if _ydf is not None and len(_ydf) >= 10:
                    ...
                    self._cache_market_data(cache_key, symbol, interval, _ydf, source="yf_compat_quick")
        ...
        try:
            ...
            if df is not None and not df.empty:
                df = self._clean(df)
                self._cache_market_data(cache_key, symbol, interval, df, source="fallback_chain")
                return df
        except Exception as e:
        ...
        try:
            ...
            if _dhan_ok():
                ...
                if _dhan_df is not None and not _dhan_df.empty:
                    ...
                    self._cache_market_data(cache_key, symbol, interval, _dhan_df, source="dhan")
        ...
        try:
            ...
            if _zd_ok():
                ...
                if _zd is not None and not _zd.empty:
                    _zd = self._clean(_zd)
                    self._cache_market_data(cache_key, symbol, interval, _zd, source="zerodha")
        ...
        try:
            ...
            if _upstox_df is not None and len(_upstox_df) >= 5:
                ...
                self._cache_market_data(cache_key, symbol, interval, _upstox_df, source="upstox")
        ...
        try:
            ...
            if _bhav_df is not None and len(_bhav_df) >= 5:
                ...
                self._cache_market_data(cache_key, symbol, interval, _bhav_df, source="bhavcopy")
            ...
            try:
                ...
                if _r.status_code == 200:
                    ...
                    if _recs and len(_recs) >= 5:
                        ...
                        if len(_idx_df) >= 5:
                            self._cache_market_data(cache_key, symbol, interval, _idx_df, source="nse_index")
        ...
        try:
            ...
            if _yf_df is not None and not _yf_df.empty and len(_yf_df) >= 5:
                ...
                self._cache_market_data(cache_key, symbol, interval, _yf_df, source="yf_compat")
        ...
        try:
            ...
            if _ext is not None and not _ext.empty:
                ...
                self._cache_market_data(cache_key, symbol, interval, _ext, source="extended_fallback")
                return _ext
