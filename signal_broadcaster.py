...
def _env_int(name: str, default: int) -> int:
    try:
        return int(str(__import__("os").getenv(name, str(default))).split("#")[0].strip())
    except Exception:
        return int(default)
...
def _env_float(name: str, default: float) -> float:
    try:
        return float(str(__import__("os").getenv(name, str(default))).split("#")[0].strip())
    except Exception:
        return float(default)


_MAX_SIGNALS_PER_DAY = _env_int("MAX_BROADCAST_SIGNALS_PER_DAY", 50)
_MIN_SCORE_BROADCAST = _env_float("MIN_SCORE_BROADCAST", 5.5)
_MIN_SCORE_PREMIUM = _env_float("MIN_SCORE_PREMIUM", 7.0)
