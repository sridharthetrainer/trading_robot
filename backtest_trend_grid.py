"""
backtest_trend_grid.py

Robust grid search for backtest_trend.backtest_trend()

Improvements
------------
- Fully aligned with backtest_trend() signature
...
RESULTS_CSV = "grid_search_results_trend.csv"
BEST_PARAMS_JSON = "best_params_trend.json"
FAILED_PARAMS_CSV = "grid_search_failed_trend.csv"


def build_param_grid() -> Dict:
    return {
        "fast_ema": [7, 9, 12],
        "slow_ema": [18, 21, 26, 34],
...
def validate_param_combo(params: Dict) -> Tuple[bool, str]:
    if params["fast_ema"] >= params["slow_ema"]:
        return False, "fast_ema_must_be_less_than_slow_ema"
...
def save_best_params(best_params: Dict, best_result: Dict, symbol: str) -> None:
    ...
    logger.info("Best trend parameters saved to %s", BEST_PARAMS_JSON)
...
def main() -> None:
    ...
    logger.info("Starting trend grid search | combinations=%d", len(all_combinations))
    ...
    for combo in tqdm(all_combinations, desc="Trend Grid Search"):
        ...
        try:
            ...
            logger.exception("Trend grid-search failure for params=%s", params)
            failed_rows.append({
                **params,
    ...
    if not results:
        logger.error("No valid trend grid-search results generated.")
        if failed_rows:
    ...
    logger.info("Saved trend grid results to %s", RESULTS_CSV)

    if failed_rows:
        pd.DataFrame(failed_rows).to_csv(FAILED_PARAMS_CSV, index=False)
        logger.info("Saved failed trend combinations to %s", FAILED_PARAMS_CSV)

    if best_params is None or best_result is None:
        logger.error("Trend grid search completed but no best result was found.")
        raise SystemExit(1)  # safe in threads

    save_best_params(best_params, best_result, symbol)

    print("\n" + "=" * 70)
    print("BEST TREND GRID RESULT")
    print("=" * 70)
