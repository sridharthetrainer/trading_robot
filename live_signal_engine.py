...
class LiveSignalEngine:
    ...
    def _run_cycle(self) -> None:
        ...
        if candidates:
            ...
            for _c in candidates:
                try:
                    from sector_rotation_engine import get_symbol_sector
                    _sect = get_symbol_sector(_c.get("symbol",""))
                except Exception:
                    _sect = "UNKNOWN"
                _sector_count[_sect] = _sector_count.get(_sect, 0) + 1
                if _sector_count[_sect] <= 2:  # max 2 per sector
                    _filtered.append(_c)
                else:
    ...
    def _evaluate_market_parallel(
        ...
        if _SIG_LOG_AVAIL:
            try:
                ...
                for _cand in candidates:
                    _sl.log_candidate(
                        signal           = self._candidate_signal_log_payload(_cand),
    ...
    def _execute_candidate(self, candidate: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                ...
                if 'pnl' in dir() and pnl is not None:
    ...
    def _build_execution_plan(
        ...
        trade_capital: float, style: str, df=None,
    ) -> Optional[Dict[str, Any]]:
        signal_side = str(signal.get("side", "")).upper()
    ...
    def _decide_style(
        ...
        if explicit_signal_style in ("swing", "scalping", "intraday"):
            return explicit_signal_style
