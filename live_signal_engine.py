...
SUPPORTED_OPTION_UNDERLYINGS = {
    ...
    # NIFTYNEXT50 dropped: no working candle source (not in Angel scrip master,
    # Upstox wrong-interval) and illiquid options — it never produced a tradeable
    # signal. Also removed from data_fetcher's scan universe.
}

# Tier-1 priority symbols — always scanned first, always need at least one signal
PRIORITY_SYMBOLS  = ["NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "SENSEX"]
...
class LiveSignalEngine:
            ...
            if symbol in self.option_fetchers:
                try:
                    ...
                    if option_result:
                        ...
                        try:
                            chain_decision = "chain_signal" if option_signal else "chain_neutral"
                            chain_side = ""
                            if isinstance(option_signal, dict):
                                sig_name = str(option_signal.get("signal", "") or "")
                                chain_side = "BUY" if sig_name == "BUY_CALL" else "SELL" if sig_name == "BUY_PUT" else ""
                            self._record_option_decision_safe(
                                symbol=symbol,
                                signal={
                                    "strategy": "option_chain_intelligence",
                                    "side": chain_side,
                                    "score": _safe_float((option_signal or {}).get("score"), 0.0)
                                    if isinstance(option_signal, dict) else 0.0,
                                },
                                decision=chain_decision,
                                reason=str((option_signal or {}).get("reason", "option_chain_snapshot"))
                                if isinstance(option_signal, dict) else "no_actionable_chain_signal",
                                side=chain_side,
                                spot=_safe_float(getattr(option_result, "spot", 0.0), 0.0),
                                quality={
                                    "source": getattr(getattr(self.option_fetchers.get(symbol), "last_source", None), "__str__", lambda: "")(),
                                    "signal": option_signal or {},
                                    "summary": option_summary or {},
                                    "expiry": getattr(option_result, "expiry", ""),
                                    "atm_strike": getattr(option_result, "atm_strike", 0),
                                },
                            )
                        except Exception:
                            logger.debug("option chain journal failed for %s", symbol)
                except Exception:
    ...
    def _candidate_signal_log_payload(self, candidate: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_price = self._first_present(
            signal.get("target"),
        ...
        if rr_f <= 0 and entry_price_f > 0 and stop_loss_f > 0 and target_price_f > 0:
            risk = abs(entry_price_f - stop_loss_f)
            reward = abs(target_price_f - entry_price_f)
            rr_f = reward / risk if risk > 0 else 0.0
        ...
        try:
            ...
            if _SIG_LOG_AVAIL and trade_id:
                try:
                    ...
                    _risk_entry = _safe_float(execution_plan.get("entry_price"), 0.0)
                    _risk_stop = _safe_float(execution_plan.get("stop_loss"), 0.0)
                    _risk_target = _safe_float(execution_plan.get("target_price"), 0.0)
                    _risk_amt = abs(_risk_entry - _risk_stop)
                    _risk_rr = (
                        abs(_risk_target - _risk_entry) / _risk_amt
                        if _risk_entry > 0 and _risk_stop > 0 and _risk_target > 0 and _risk_amt > 0
                        else 0.0
                    )
                    option_meta = {
