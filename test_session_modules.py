...
def test_macro_sector_map_rules():
    ...
    s = mg.sector_impact_map({"BRENT": 2.0, "USDINR": 0.4, "NASDAQ_FUT": 0.8})
    assert "OMCs" in s["negative_sectors"] and "IT" in s["positive_sectors"]
...
def test_macro_log_schema(tmp_path):
    import macro_global_profit_engine as mg
    db = str(tmp_path / "macro.db")
    ctx = {"timestamp": "2026-06-20T16:00:00", "gift_nifty_change": 0.45,
           "final_global_score": 62.0, "us_futures_score": 40.0,
           "asia_score": 20.0, "europe_score": 10.0, "commodity_score": -5.0,
           "currency_score": 3.0, "vix_score": 2.0, "bond_yield_score": 1.0,
           "gap_prediction": "GAP_UP", "gap_probability": 61.0,
           "nifty_bias": "Strong Bullish Day", "banknifty_bias": "Strong Bullish Day",
           "market_regime": "GLOBAL_RISK_ON", "profit_quality_score": 82,
           "allowed_trade_type": "CE", "no_trade_reason": "",
           "reasons": ["GIFT +0.45%"]}
    assert mg.log_sentiment(ctx, db_path=db) is True
    con = sqlite3.connect(db)
    n = con.execute("SELECT COUNT(*) FROM macro_global_sentiment").fetchone()[0]
    cols = [r[1] for r in con.execute("PRAGMA table_info(macro_global_sentiment)").fetchall()]
    assert n == 1
    assert "final_global_score" in cols and "profit_quality_score" in cols


def test_macro_profit_filter_allows_aligned_ce(monkeypatch):
    import macro_global_profit_engine as mg
    assets = {
        "GIFT": {"change_pct": 0.6, "price": 25000},
        "SP500_FUT": {"change_pct": 0.7},
        "NASDAQ_FUT": {"change_pct": 0.8},
        "NIKKEI": {"change_pct": 0.5},
        "DAX": {"change_pct": 0.4},
        "BRENT": {"change_pct": -0.8},
        "USDINR": {"change_pct": -0.2},
        "DXY": {"change_pct": -0.2},
        "INDIAVIX": {"price": 14, "change_pct": -1.0},
        "US10Y": {"change_pct": -1.0},
        "COPPER": {"change_pct": 0.8},
    }
    monkeypatch.setattr(mg, "fetch_macro_assets", lambda force=False: assets)
    res = mg.apply_macro_profit_filter(
        {"side": "BUY"},
        technical_context={"cpr_bias": "ABOVE_CPR", "vwap_position": "ABOVE_VWAP",
                           "volume_ratio": 1.5, "rr": 2.0, "oi_direction": "CE"},
        option_data={"iv_percentile": 45, "theta_risk": 0.2},
        force=True,
    )
    assert res["trade_permission"] == "ALLOW"
    assert res["allowed_direction"] in {"CE", "BOTH"}
    assert res["profit_quality_score"] >= 75


def test_macro_profit_filter_blocks_sideways(monkeypatch):
    import macro_global_profit_engine as mg
    monkeypatch.setattr(mg, "fetch_macro_assets", lambda force=False: {
        "GIFT": {"change_pct": 0.02, "price": 25000},
        "SP500_FUT": {"change_pct": -0.01},
        "INDIAVIX": {"price": 13, "change_pct": 0.0},
    })
    res = mg.apply_macro_profit_filter(
        {"side": "BUY"},
        technical_context={"cpr_bias": "INSIDE_CPR", "volume_ratio": 0.8},
        option_data={"iv_percentile": 30},
        force=True,
    )
    assert res["trade_permission"] == "BLOCK"
    assert res["allowed_direction"] == "NONE"


def test_macro_trap_detection_blocks_bull_trap(monkeypatch):
    import macro_global_profit_engine as mg
    monkeypatch.setattr(mg, "fetch_macro_assets", lambda force=False: {
        "GIFT": {"change_pct": 0.7, "price": 25000},
        "SP500_FUT": {"change_pct": 0.5},
        "INDIAVIX": {"price": 16, "change_pct": 0.0},
    })
    res = mg.apply_macro_profit_filter(
        {"side": "BUY"},
        technical_context={"cpr_bias": "BELOW_CPR", "failed_resistance": True,
                           "volume_ratio": 0.5},
        option_data={"ce_premium_decays": True, "iv_percentile": 35},
        force=True,
    )
    assert res["trade_permission"] == "BLOCK"
    assert "trap" in res["risk_warning"].lower()
...
# ── FII/DII tradeReact parser (new — critical, fragile NSE endpoint) ──────────
def test_fii_tradereact_parse(monkeypatch):
    import fii_data_fetcher as f

    class _Resp:
        status_code = 200
        def json(self):
            return [
                {"buyValue": "17274.01", "category": "DII", "date": "24-Jun-2026",
                 "netValue": "3637.26", "sellValue": "13636.75"},
                {"buyValue": "16744.73", "category": "FII/FPI", "date": "24-Jun-2026",
                 "netValue": "-1843.4", "sellValue": "18588.13"},
            ]
    ...
    out = f._fetch_fiidii_tradereact()
    assert out.get("fii_net") == -1843.4 and out.get("dii_net") == 3637.26
    assert out.get("source") == "NSE_FIIDII_TRADEREACT"
...
def test_option_scalper_signal_and_gating():
    ...
    assert o.scalp_signal(flat) is None              # no breakout -> no signal
    c = [100] * 19 + [104]
    up = pd.DataFrame({"open": c, "high": [101] * 19 + [104.5],
                       "low": [99] * 19 + [103], "close": c,
                       "volume": [1000] * 19 + [3000]}, index=idx)
    sig = o.scalp_signal(up)
    assert sig and sig["side"] == "BUY" and sig["option_type"] == "CE"
    assert o._atm_strike(25013, "NIFTY") == 25000    # ATM rounding
    assert o._atm_strike(51240, "BANKNIFTY") == 51200
    assert o.maybe_scan() == 0                        # disabled by default (no env)
