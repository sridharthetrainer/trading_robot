...
_DB_FILE    = Path("participant_oi.db")   # archive-backed (participant_oi_backfill.py)
...
# ── Archive-backed positioning (real published data) ──────────────────────────

# DB client_type label -> our category key
_DB_CTYPE = {"FII": "FII", "DII": "DII", "Pro": "PRO", "Client": "CLIENT"}


def _has_positioning(data: dict) -> bool:
    """True if FII index-futures positioning is non-zero (i.e. live fetch worked)."""
    fii = (data or {}).get("FII", {})
    return (float(fii.get("fut_long", 0) or 0) + float(fii.get("fut_short", 0) or 0)) > 0


def _load_latest_from_db() -> dict:
    """Map the most-recent participant_oi.db row (per client_type) into the dict
    shape compute_participant_signal expects. Uses INDEX futures/options (the
    canonical FII directional read). Returns {} if the DB is absent/empty.

    Participant OI is EOD/daily data (published ~8pm, entry_lag >= 1), so the
    latest stored day is the correct read, not a "stale" value to discard.
    """
    ...
    try:
        ...
        try:
            row = conn.execute("SELECT MAX(date) FROM participant_oi").fetchone()
            latest = row[0] if row else None
            if not latest:
                return {}
            rows = conn.execute(
                "SELECT client_type, future_index_long, future_index_short, "
                "opt_index_call_long, opt_index_call_short, "
                "opt_index_put_long, opt_index_put_short "
                "FROM participant_oi WHERE date=?", (latest,)).fetchall()
        finally:
            conn.close()
    except Exception as e:
        logger.debug("participant_oi DB read: %s", e)
        return {}
    ...
    for ctype, fl, fs, cl, cs, pl, ps in rows:
        cat = _DB_CTYPE.get(str(ctype).strip())
        if not cat:
            continue
        out[cat]["fut_long"]   = float(fl or 0)
        out[cat]["fut_short"]  = float(fs or 0)
        out[cat]["call_long"]  = float(cl or 0)
        out[cat]["call_short"] = float(cs or 0)
        out[cat]["put_long"]   = float(pl or 0)
        out[cat]["put_short"]  = float(ps or 0)
    return out
...
def get_participant_data(force: bool = False) -> dict:
    ...
    The live NSE JSON endpoints are unreliable and currently return nothing, so
    positioning is taken from the archive-backed DB (participant_oi.db, kept
    fresh by participant_oi_backfill.py / eod_market_capture). FII/DII cash flow
    still comes from the live fetch when available; net_cash otherwise stays 0
    (the engine's cash context reads fii_data_fetcher separately).

    LIMITATION (audit #13): there is NO intraday FII/DII F&O positioning — the
    only positioning available is EOD (published ~8pm, entry_lag >= 1). Any
    consumer must treat FII positioning as day-stale, never as a live intraday
    signal.
    """
    if not force:
        try:
    ...
    if not _has_positioning(data):
        db = _load_latest_from_db()
        if db:
            # keep any real cash the live fetch produced; take positioning from DB
            for cat in ("FII", "DII"):
                cash = float((data.get(cat, {}) or {}).get("net_cash", 0) or 0)
                if cash:
                    db.setdefault(cat, {})["net_cash"] = cash
            data = db
    if data:
        try:
