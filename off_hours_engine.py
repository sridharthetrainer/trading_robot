...
def fetch_nse_holidays() -> set:
    ...
    Hard-bounded: requests' `timeout=` does NOT cover DNS resolution
    (getaddrinfo), so a broken resolver — e.g. right after a suspend/resume —
    can hang these GETs for minutes despite the per-request timeouts. This runs
    SYNCHRONOUSLY in startup, so an unbounded hang here would stall the whole bot
    (the same class of failure that froze startup on 2026-06-23). The fetch runs
    in a daemon thread joined with a wall-clock cap; on timeout we fall back to
    the built-in NSE_HOLIDAYS_2026 list.
    """
    box: dict = {}
    ...
    t.start()
    t.join(20)   # inner timeouts sum to 16s; cap with margin, then fall back
    if t.is_alive():
        logger.warning("NSE holiday fetch exceeded 20s (DNS?) — using built-in holiday list")
    holidays = box.get("res")
    if holidays:
