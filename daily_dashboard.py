...
def _data_freshness_section() -> List[str]:
    """EOD data freshness (stale → nightly gap-filler backfills it)."""
    try:
        import data_gap_filler
        rows = data_gap_filler.freshness()
        out = ["🗓️ EOD DATA FRESHNESS"]
        for r in rows:
            sd = r.get("stale_days")
            flag = "✅" if (sd is not None and sd <= 1) else "⚠️"
            out.append(f"  {flag} {r['name']}: last={r.get('last')} (stale {sd}d)")
        if any((r.get("stale_days") or 0) > 1 for r in rows):
            out.append("  → nightly gap-filler backfills; or run data_gap_filler.py --fill")
        return out
    except Exception:
        return ["🗓️ EOD DATA FRESHNESS", "  (unavailable)"]
