...
# Set DISABLE_YFINANCE=true in .env to skip the Yahoo Finance fallback (delayed + unreliable)
_DISABLE_YFINANCE = os.getenv("DISABLE_YFINANCE", "false").lower() in ("true", "1", "yes")
...
def get_market_data_from_pool(
    ...
    sources = [
        ...
        ("yfinance",     lambda: _from_yfinance(symbol, interval, days)) if not _DISABLE_YFINANCE else None,
    ...
    for name, fn in [s for s in sources if s is not None]:
        try:
            df = fn()
