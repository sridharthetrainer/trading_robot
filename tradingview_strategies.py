...
def run_chaikin_mf_strategy(
    ...
    try:
        ...
        spread = h - l
        # np.where still evaluates the division for spread==0 elements
        # (RuntimeWarning every scan); np.divide with where= skips them
        mfm    = np.divide((c - l) - (h - c), spread,
                           out=np.zeros_like(spread, dtype=float),
                           where=spread > 0)
        mfv    = mfm * v
