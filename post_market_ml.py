...
Run after 15:30 IST every trading day:
  1. Load labelled signals from signal_log.db  (last 90 days)
...
Usage:
    python3 post_market_ml.py                  # full run, all symbols, 90 days
    python3 post_market_ml.py --days 30        # shorter lookback
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            try:
                matrix.record_trade(
                    strategy    = strategy,
                    pnl         = float(outcome),  # tb_label proxy: +1 win / -1 loss / 0 timeout
                    day_type    = day_type,
...
def run_pipeline(
    ...
    Parameters
    ----------
    days           : lookback for signal_log (default 90 days)
...
def main() -> int:
    ...
    ap.add_argument("--days",        type=int,  default=90,
                    help="Lookback days for signal_log (default 90)")
    ap.add_argument("--candle-days", type=int,  default=30,
                    help="Days of candle-augmented features (default 30)")
