...
def _get_news_sentiment(symbol: str) -> float:
    ...
    Uses the working OmniSource per-symbol score (old news_sentiment /
    market_intelligence_hub imports were dead)."""
    try:
        from omnisource_news_engine import get_symbol_specific_news
        return float((get_symbol_specific_news(symbol) or {}).get("score_adj", 0) or 0)
    except Exception:
        return 0.0


def _get_vix() -> float:
    """Live India VIX via the 4-source resilience chain (old market_regime /
    config imports were dead → always returned the 15.0 default)."""
    try:
        from data_source_resilience import get_india_vix
        v = float(get_india_vix() or 0)
        if v > 0:
            return v
    except Exception:
        pass
