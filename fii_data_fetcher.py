...
def _fetch_fiidii_tradereact() -> Dict:
    """NSE /api/fiidiiTradeReact — the FII/DII endpoint that actually returns data
    on this IP (the /api/fiidii + participant endpoints below 404/empty here, but
    the system-health check confirmed tradeReact returns FII+DII cash rows).
    Returns net/buy/sell in ₹ Cr."""
    try:
        import requests
        s = requests.Session()
        try:
        ...
        if not isinstance(rows, list) or not rows:
            return {}
        out: Dict = {"date": date.today().isoformat(), "source": "NSE_FIIDII_TRADEREACT"}
        for row in rows:
            ...
            if "FII" in cat or "FPI" in cat:
                out.update({"fii_net": net, "fii_buy": buy, "fii_sell": sell})
            elif "DII" in cat:
                out.update({"dii_net": net, "dii_buy": buy, "dii_sell": sell})
        if "fii_net" in out or "dii_net" in out:
            return out
    except Exception as e:
        logger.debug("fiidiiTradeReact: %s", e)
    return {}
...
def fetch_nse_fii_dii_today() -> Dict:
    ...
    """
    # Source 0: /api/fiidiiTradeReact — the endpoint that works on this IP.
    data0 = _fetch_fiidii_tradereact()
    if data0 and (data0.get("fii_net") is not None or data0.get("dii_net") is not None):
        return data0
    try:
        import requests
        s = requests.Session()
