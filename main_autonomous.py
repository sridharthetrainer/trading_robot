...
MIN_LIVE_CAPITAL = float(__import__("os").getenv("MIN_LIVE_CAPITAL","25000"))
...
class AutonomousTradingSystem:
    ...
    def _after_hours_tasks(self) -> None:
        ...
        try:
            if _ADV_LIVE_AVAILABLE if '_ADV_LIVE_AVAILABLE' in dir() else False:
                _get_global_macro().fetch()
        except Exception as _e:
            import logging; logging.getLogger(__name__).debug("suppressed: %s", _e)
        try:
            ...
            if _MONITOR_AVAIL and self._conn_monitor:
                try:
                    ...
                    if __import__("time").time() - _last > 1800:
                        ...
                        def _mid_check():
                            _res = self._conn_monitor.run_critical_only()
                            self._conn_monitor.send_alert_if_broken(_res, "mid-session")
    ...
    def run(self) -> None:
        ...
        # Start Google Drive sync watcher
        if _DRIVE_SYNC_AVAIL:
            try:
