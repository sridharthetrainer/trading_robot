...
class AlertManager:
    def __init__(
        ...
        self._dedup_file = Path("dedup_state.json")
        self._dedup_sent: Dict[str, float] = self._load_dedup()
        self._spool_file = Path("telegram_spool.jsonl")
        ...
        for attempt in range(3):
            try:
                ...
                if resp.status_code not in (408, 429, 500, 502, 503, 504):
                    logger.debug("Telegram non-retryable status=%s body=%s", resp.status_code, resp.text[:120])
                    return False
            except Exception as exc:
                logger.debug("Telegram send attempt %d failed: %s", attempt + 1, exc)
    ...
    def status_15min(
        ...
        **kwargs,
        ...
        # Signal counts
        sig_parts = [f"Scanned: {symbols_scanned}"]
        if signals_found:  sig_parts.append(f"Signals: {signals_found}")
        if tier1_signals:  sig_parts.append(f"Strong: {tier1_signals}")
        L.append("  📊 " + "  ·  ".join(sig_parts))

        # P&L line
        if daily_pnl is not None:
            bar = self._bar(abs(daily_pnl), daily_limit)
            ur  = f"  Unreal ₹{unrealized_pnl:+,.0f}" if unrealized_pnl else ""
            L.append(f"  {pi} P&L ₹{daily_pnl:+,.0f}{ur}  {bar}")

        # Positions
        if open_positions:
            L.append(f"  📂 Positions: {open_positions} open")
            if open_pos_list:
                for pos in open_pos_list[:2]:
                    sym = pos.get("symbol","?")
        ...
        if expiry_day:
            L.append(f"  📆 EXPIRY DAY — 0DTE mode active")

        L.append(f"  📱 /signals · /positions · /pnl")
