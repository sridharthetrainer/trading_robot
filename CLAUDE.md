...
2. Widen instrumentation to still-unlogged inputs. DONE (2026-06-20, commits
   ab29b8fe / 6b18662d / 3b2985d8). The modifiers that were computed-then-
   DISCARDED before logging are now captured end-to-end (signal_engine
   `_sig_meta`/`_cand_meta` -> live_signal_engine harvest key-list ->
   signal_log column + `_new_cols` migration; live DB migrated, bot restarted):
   - `weinstein_mod` — producer DID exist (`weinstein_stage_filter`, applied to
     adjusted_score at signal_engine ~3979); the prior "no producer" note was
     wrong — the value was just discarded. Now logged.
   - `sector_mod` — sector-rotation `score_mod` (signal_engine ~3561). Now logged.
   - `crsi_mod` / `nr_mod` / `volume_mod` — inline connors_rsi / nr7 / volume
     bonuses (signal_engine ~3332/3368/3384), logged as the applied deltas.
   These are now MEASURABLE by `modifier_edge_analyzer` once live days accrue —
   instrumentation removes blind spots, it does NOT create edge; verdicts stay
   unmeasured until data accrues. Still not independent signals (unchanged):
   `rl_bias` has a live producer (`_rl_score_adjustment`); `ai_score` is
   redundant with confidence (`signal_log` writes `ai_score = signal.confidence`,
   signal_log.py:279).
