...
def test_good_chain_passes():
    ...
    assert q.ok and q.reason == "ok_to_buy_options" and q.straddle_price == 200


def test_stale_chain_blocks():
    data = {
        "timestamp": time.time() - 600,
        "records": {"underlyingValue": 20000, "data": [_row(20000)]},
    }
    q = evaluate_option_buy_quality(data, max_chain_age_sec=120)
    assert not q.ok and q.reason == "option_chain_stale"


def test_expected_move_consumed_blocks():
    data = {
        "timestamp": time.time(),
    ...
    assert not q.ok and q.reason == "expected_move_already_consumed"


def test_wide_atm_spread_blocks():
    data = {
        "timestamp": time.time(),
        "records": {"underlyingValue": 20000, "data": [_row(20000, bid=50, ask=100)]},
    }
    q = evaluate_option_buy_quality(data, max_atm_spread_pct=0.20)
    assert not q.ok and q.reason == "atm_spread_too_wide"
