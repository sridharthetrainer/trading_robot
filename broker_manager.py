...
def _broker_accepts(func: Any, param: str) -> bool:
    """True if a broker method accepts the named keyword (or **kwargs).

    Lets us forward the SEBI order_tag only to brokers that support it,
    so a broker without the parameter never raises TypeError.
    """
    try:
...
class BrokerManager:
    ...
    def reconnect_unusable(self) -> int:
        """Re-login any broker that has dropped (e.g. Angel obj=None after a
        startup login rate-limit, which otherwise degrades the WHOLE session — it
        lost Friday 06-26). Clears our health cooldown then calls broker.connect();
        connect() has its own rate-limit breaker so this is safe to call on a
        timer. Returns the number recovered. Best-effort, never raises."""
        recovered = 0
        for broker in self.brokers:
            try:
                if self._is_usable(broker):
                    continue
                if not hasattr(broker, "connect"):
                    continue
                st = self.health.get(self._broker_name(broker))
                if st:
                    st.cooldown_until = 0.0   # allow a fresh login attempt
                if broker.connect() and self._is_usable(broker):
                    recovered += 1
                    if st:
                        st.failures = 0
                    logger.info("Broker reconnected: %s", self._broker_name(broker))
            except Exception as exc:
                logger.debug("reconnect %s: %s", self._broker_name(broker), exc)
        return recovered
    ...
    def place_order_with_fallback(
        ...
        try:
            if stop_loss and target and hasattr(broker, "place_bracket_order"):
                ...
                # Forward the SEBI algo-order tag only to brokers that accept it
    ...
    def place_order(
        ...
        order_id, broker_name = self.place_order_with_fallback(
            ...
            price=price, exchange=exchange, order_tag=order_tag,
        )
