...
try:
    ...
    # Count strategies in STRATEGIES list
    m = re.search(r"STRATEGIES\s*=\s*\[(.*?)\]", se_src, re.DOTALL)
    if m:
        strat_names = re.findall(r"run_\w+_strategy", m.group(1))
        conditional = re.findall(r"\*\(\[_\w+", m.group(1))
        total_est = len(strat_names) + len(conditional) + 2  # +2 for Williams 3-pack
        ok(f"STRATEGIES list has ~{total_est} strategies")
    else:
        fail("STRATEGIES list not found in signal_engine.py")
except Exception as e:
