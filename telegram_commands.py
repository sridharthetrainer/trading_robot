...
class TelegramCommandHandler:
    ...
    def send_photo(self, photo_path: str, caption: str = "", chat_id: str = None) -> bool:
        ...
        try:
            ...
            if r.status_code == 200:
                return True
            logger.warning("Telegram sendPhoto failed status=%s body=%s", r.status_code, r.text[:160])
    ...
    def start(self) -> None:
        ...
        try:
            me = self._api("getMe")
            if not me.get("ok"):
                desc = str(me.get("description", "unknown error"))
                code = int(me.get("error_code") or 0)
                lower_desc = desc.lower()
                auth_error = code in {401, 403, 404} or any(
                    marker in lower_desc
                    for marker in ("unauthorized", "forbidden", "not found", "token")
                )
                if auth_error:
                    logger.error(
                    ...
                    logger.warning(
                        "Telegram command handler offline: connectivity/API validation failed: %s",
                        desc,
                    )
                return
    ...
    def _register_defaults(self) -> None:
        ...
        self.register("globalbias",  self._cmd_globalbias)
        ...
        self.register("oichart",     self._cmd_oi_chart)
        ...
        self.register("oiprofile",   self._cmd_oi_sr)
        self.register("chainsignals", self._cmd_chain_signals)
        self.register("chains",       self._cmd_chain_signals)
    ...
    def _cmd_schedule(self, _="") -> str:
        ...
        lines = [
            ...
            "  🔜 17:30  ML model training (15d signal log)",
    ...
    def _cmd_chain_signals(self, args: str = "") -> str:
        """Recent option-chain directional reads (regime/bias/gamma) — logged as
        'chain_signal' decisions. These are context reads, not trade selections."""
        try:
            import json as _cj
            from option_decision_journal import DEFAULT_JOURNAL_FILE
            want = (str(args or "").split()[0].upper() if str(args or "").strip() else "")
            hits = []
            try:
                with open(DEFAULT_JOURNAL_FILE) as f:
                    for ln in f:
                        try: d = _cj.loads(ln)
                        except Exception: continue
                        if d.get("decision") != "chain_signal":
                            continue
                        if want and str(d.get("symbol", "")).upper() != want:
                            continue
                        hits.append(d)
            ...
            hits = hits[-6:]
            if not hits:
                return "No chain signals yet today."
            out = ["🧭 <b>Recent option-chain signals</b>",
                   "<i>(directional reads, not trade selections)</i>"]
            for d in hits:
                short = " | ".join(str(d.get("reason", "")).split(" | ")[:3])
                out.append(f"  {str(d.get('time',''))[11:16]}  {d.get('symbol')} "
                           f"<b>{d.get('side')}</b> @ {d.get('spot')}\n    {short}")
            return "\n".join(out)
        except Exception as e:
            return f"/chainsignals error: {e}"

    def _cmd_oi_sr(self, args: str = "") -> str:
        """OI support/resistance strike-profile chart — top strikes near spot by OI.
          /oisr            (NIFTY)
          /oisr BANKNIFTY  /oisr NIFTY 16"""
        try:
            toks = [t.strip() for t in str(args or "").split() if t.strip()]
            sym = toks[0].upper() if toks else "NIFTY"
            n = int(toks[1]) if len(toks) > 1 and toks[1].isdigit() else 12
            from option_oi_chart import generate_oi_strike_profile_chart
            res = generate_oi_strike_profile_chart(sym, n_strikes=n)
            if getattr(res, "ok", False) and res.path:
                self.send_photo(res.path, caption=res.caption)
                return ""
            return f"⚠️ OI S/R: {getattr(res, 'reason', 'unavailable')}"
        except Exception as e:
            return f"/oisr error: {e}"

    def _cmd_oi_chart(self, args: str = "") -> str:
        """
        Send intraday OI line chart for a day.
        Usage:
          /oichart NIFTY 2026-06-18
          /oichart NIFTY 2026-06-18 23500
          /oichart NIFTY 2026-06-18 top5
          /oichart NIFTY 2026-06-18 23500,23550,23600
        """
        try:
            import re
            from datetime import datetime as _dt
            ...
            for token in tokens:
                ...
                if up in valid:
                    sym = up
                    continue
                if up.startswith("TOP") and up[3:].isdigit():
                    compare_top = max(2, min(8, int(up[3:])))
                    continue
                if "," in token:
                    parsed = []
                    ...
                    if parsed:
                        strikes.extend(parsed)
                    continue
                if re.match(r"^\d{4}-\d{2}-\d{2}$", token):
                    day = token
                    continue
                if re.match(r"^\d{1,2}-\d{1,2}-\d{4}$", token):
                    day = _dt.strptime(token, "%d-%m-%Y").strftime("%Y-%m-%d")
                    continue
            ...
            if not result.ok:
                return (
                    f"📉 <b>OI chart unavailable</b>\n"
                    f"Symbol: {sym}\n"
                    f"Date: {day}\n"
                    f"Reason: {result.reason}\n\n"
                    f"Need at least 2 option-chain snapshots for that day.\n"
                    f"Try: <code>/oichart NIFTY {day} top5</code>"
                )

            if self.send_photo(result.path, result.caption):
                return ""
            return f"⚠️ Chart generated but Telegram photo send failed:\n<code>{result.path}</code>"
        except Exception as e:
            return f"❌ OI chart error: {str(e)[:80]}"
    ...
    def _cmd_strikeflow(self, args: str = "") -> str:
        """
        Top traded CE/PE strikes by OI, OI change, and volume.
        Usage:
          /strikeflow
          /strikeflow BANKNIFTY
          /strikeflow NIFTY 8
          /strikeflow SENSEX snapshot
        """
        try:
            tokens = [t.strip() for t in str(args or "").split() if t.strip()]
            valid = {"NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "SENSEX"}
            ...
            for token in tokens:
                ...
                if up in valid:
                    sym = up
                elif up in {"SNAP", "SNAPSHOT", "CACHE", "STORED"}:
                    prefer_live = False
                elif token.isdigit():
                    top_n = max(2, min(8, int(token)))
    ...
    def _cmd_globalbias(self, args: str = "") -> str:
        ...
        Not a signal/prediction — surfaces existing cross_asset/global_filter."""
        try:
            from macro_global_profit_engine import format_globalbias
            return format_globalbias().replace("<b>", "").replace("</b>", "")
        except Exception as e:
            return f"⚠️ /globalbias unavailable: {str(e)[:80]}"
