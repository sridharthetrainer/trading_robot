...
# ── Config ────────────────────────────────────────────────────────────────
POLL_INTERVAL       = 30          # check order book every 30 seconds
UPDATE_INTERVAL     = 900         # send P&L update every 15 minutes
...
class ManualTradeTracker:
    ...
    def analyze_trade(self, trade: ManualTrade) -> ManualTrade:
        ...
        alignment = len(bullish) if trade.side == "BUY" else len(bearish)
    ...
    def send_trade_detected(self, trade: ManualTrade):
        ...
        alignment = bull_count if is_long else bear_count
...
{'━' * 32}"""
  ...
  Strategy accuracy: {correct}/{bull + bear} aligned"""
    ...
    def _adjust_protection(self, trade: ManualTrade, ltp: float) -> None:
        ...
        # Baseline: simple profit ratchet (cheap, always available)
        slp = OPTION_SL_PCT if self._is_option(trade) else EQUITY_SL_PCT
        ...
        while self._running:
            try:
                ...
                for trade in new_trades:
                    ...
                    # 4b. Place broker-side GTT SL + target (survives a crash)
                    self._place_protection(trade)
                    # 5. Add to active tracking
                    self._active_trades[trade.order_id] = trade
