...
class ManualTradeTracker:
    ...
    def _angel_response_invalid(self, response: object, source: str) -> bool:
        """Detect expired/broken SmartAPI responses and force reconnect.

        SmartAPI sometimes logs AG8001 internally and returns None to callers,
        so checking only the response dict leaves the tracker "angel=up" but
        blind to order/position books.
        """
        if response is None:
            ...
            return False
        logger.warning(
            "Angel response invalid during %s (%s) — forcing manual tracker reconnect",
            source, reason,
        )
        try:
            if self._angel:
                self._angel.obj = None
        except Exception:
            pass
        self._last_angel_attempt = 0.0
        self._ensure_angel()
        return True
    ...
    def poll_order_book(self) -> List[ManualTrade]:
        ...
        try:
            ...
            if self._angel_response_invalid(book, "orderBook"):
                return new_trades
  ...
  Strategy accuracy: {correct}/{bull + bear} aligned"""
    ...
    def check_exits(self):
        ...
        try:
            ...
            if self._angel_response_invalid(positions, "position/check_exits"):
                return
    ...
    def sync_open_positions(self, extra_symbols: Optional[set] = None) -> List[ManualTrade]:
        ...
        try:
            with self._angel._lock:
                positions = self._angel.obj.position()
            if self._angel_response_invalid(positions, "position/sync_open_positions"):
                return new_trades
            if not positions or not positions.get("data"):
                return new_trades
