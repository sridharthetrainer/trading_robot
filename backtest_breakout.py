...
slippage_model = SlippageModel(percent_slippage=0.01, min_slippage_amount=0.5)  # NSE NIFTY ~0.01%
...
def backtest_breakout(
    ...
    if breakout_buffer_atr < 0 or min_body_atr < 0:
        raise ValueError("Invalid breakout entry filter bounds")
    ...
    for i in range(valid_start, last_bar_idx):
        ...
        buffer = breakout_buffer_atr * current_atr
        bullish_breakout = price > prev_high + buffer
        bearish_breakout = price < prev_low - buffer
        ...
        if candle_body < min_body_atr * current_atr:
            skipped_due_to_filters += 1
            continue
