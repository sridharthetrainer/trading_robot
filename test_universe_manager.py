...
def test_learning_universe_keeps_indices_first():
    old = {
    ...
    try:
        ...
        assert universe[:3] == ["NIFTY", "BANKNIFTY", "FINNIFTY"] and "RELIANCE" in universe
    finally:
        _restore(old)


def test_probation_universe_allows_underlying_from_option_symbol():
...
def test_live_universe_empty_means_no_extra_filter():
    old = {"LIVE_UNIVERSE": _set("LIVE_UNIVERSE", [])}
    try:
        assert is_live_symbol_allowed("ANYTHING")
    finally:
        _restore(old)
