...
def _gdrive_enabled() -> bool:
    """Master switch. DISABLED by default: we edit via Claude Code / VS Code + git,
    and this module's bidirectional pull would overwrite edits with stale Drive
    copies. Set ENABLE_GDRIVE_SYNC=true to re-enable Drive-based deploy/sync."""
    return str(os.getenv("ENABLE_GDRIVE_SYNC", "false")).lower() in ("1", "true", "yes", "on")
...
class DriveSyncWatcher:
    ...
    def start(self) -> None:
        if not _gdrive_enabled():
            logger.info("Google Drive sync DISABLED (ENABLE_GDRIVE_SYNC=false; "
                        "editing via Claude Code/VS Code + git)")
            return
