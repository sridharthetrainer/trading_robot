...
Checks (all must pass for PASS verdict):
  1. Deflated Sharpe Ratio >= 0.95 — accounts for multiple grid-search trials
...
class ValidationResult:
    ...
    # Benchmark gate — PASS also requires beating a passive buy-&-hold on the
    # locked holdout (risk-adjusted, by Sharpe). None when holdout not evaluated.
    benchmark_buyhold_sharpe: Optional[float] = None
...
def buy_hold_sharpe(df: pd.DataFrame, interval_minutes: int = 5) -> Optional[float]:
    """Annualised buy-&-hold Sharpe over the same price series — a fair,
    scale-invariant benchmark (the strategy backtests are leveraged fixed-lot,
    so comparing P&L is apples-to-oranges; Sharpe is comparable). Prefers
    daily-grouped returns (√252, matching the backtests' _compute_annualised_sharpe);
    falls back to per-bar returns annualised by bars/day. Returns None if it
    cannot be computed (the caller then treats the benchmark as not-beaten)."""
    try:
        col = next((c for c in ("Close", "close", "CLOSE") if c in df.columns), None)
        if col is None or len(df) < 3:
            return None
        if isinstance(df.index, pd.DatetimeIndex):
            rets = df[col].groupby(df.index.normalize()).last().pct_change().dropna()
            ann  = 252 ** 0.5
        elif "date" in df.columns:
            key  = pd.to_datetime(df["date"], errors="coerce").dt.normalize()
            rets = df[col].groupby(key).last().pct_change().dropna()
            ann  = 252 ** 0.5
        else:
            rets = df[col].pct_change().dropna()
            ann  = (252 * _bars_per_day(df, interval_minutes)) ** 0.5
...
def run_validation(
    ...
    # Benchmark gate: a strategy must beat a passive buy-&-hold on the SAME locked
    # holdout, risk-adjusted (Sharpe). Random/SMA baselines need a generic engine
    # (TODO); buy-&-hold is the load-bearing one and needs no extra engine.
    benchmark_buyhold_sharpe = buy_hold_sharpe(holdout_df, interval_minutes) if h_sharpe is not None else None
    beats_benchmark = bool(
        h_sharpe is not None
        and benchmark_buyhold_sharpe is not None
        and h_sharpe > benchmark_buyhold_sharpe
    )
    if h_sharpe is not None:
        logger.info(
            "VH[%s] BENCHMARK: strat_holdout_sharpe=%.3f vs buy_hold=%s → beats=%s",
            strategy_name, h_sharpe,
            ("%.3f" % benchmark_buyhold_sharpe) if benchmark_buyhold_sharpe is not None else "n/a",
            beats_benchmark,
        )

    # PASS requires: strong deflated Sharpe + min trades + stable params + the
    # locked holdout independently positive (Sharpe and P&L) + beating buy-&-hold.
    holdout_ok = (h_pnl is not None and h_pnl > 0
                  and h_sharpe is not None and h_sharpe > 0)
    all_pass = dsr_ok and min_trade_ok and stab_ok and holdout_ok and beats_benchmark
    ...
    result = ValidationResult(
        ...
        verdict                = verdict,
        benchmark_buyhold_sharpe = round(benchmark_buyhold_sharpe, 4) if benchmark_buyhold_sharpe is not None else None,
        beats_benchmark        = beats_benchmark,
        dev_windows_detail     = window_results,
        best_params            = best_params or {},
    )
    # Registry (audit #9): record every run so failed ideas aren't retested.
    try:
        import experiment_registry
        experiment_registry.log_result(result, timeframe=f"{interval_minutes}m")
    except Exception:
        pass
    return result
...
if __name__ == "__main__":
    ...
    for strategy in strategies:
        ...
        print(f"  Deflated Sharpe   : {result.deflated_sharpe:.3f}  "
              f"(>=0.95 required)   → {'✅' if result.deflated_sharpe >= 0.95 else '❌'}")
