"""rectangle.py — horizontal channel / range breakout patterns."""
    ...
    def detect(self, df: pd.DataFrame, context: DetectionContext,
        ...
        for ch in channels:
            ...
            if C[brk] > upper:
                ...
                pattern = "rectangle"
                entry, stop, target, level = C[brk], lower, C[brk] + ch.width, upper
            elif C[brk] < lower:
                direction = Direction.SHORT
                pattern = "rectangle"
                entry, stop, target, level = C[brk], upper, C[brk] - ch.width, lower
            else:
                continue
            ...
            r = make_result(
                ...
                direction=direction, confidence_score=conf,
                entry=entry, stop=stop, target=target, level=level,
...
class HorizontalRangeDetector(RectangleDetector):
    name = "horizontal_range"

    def detect(self, df: pd.DataFrame, context: DetectionContext,
               symbol: str) -> List[PatternResult]:
        out = super().detect(df, context, symbol)
        for r in out:
            r.pattern = self.name
        return out
