            ...
            for row in pending:
                ...
                try:
                    ...
                    sig_ts = datetime.strptime(
                        f"{date.today() - __import__('datetime').timedelta(days=1)} {row['signal_time']}",
                        "%Y-%m-%d %H:%M:%S"
                    ).timestamp()
                    entry_idx = 0
                    ...
                    # Find peak and max adverse
                    if entry_idx < len(df) - 1:
                        future = df.iloc[entry_idx: entry_idx + max_b + 1]
                        peak   = float(future["high"].max() if "high" in df.columns
                                       else future.iloc[:, 1].max())
                        trough = float(future["low"].min()  if "low"  in df.columns
                                       else future.iloc[:, 2].min())
                        max_adv = abs(ep - trough) / ep * 100 if side == "BUY" else abs(peak - ep) / ep * 100
                    else:
                        peak = ep; max_adv = 0.0
                    ...
                    with self._conn() as conn:
                        conn.execute(
                            ...
                            f"max_adverse_move=? WHERE id=?",
                            (label, peak, round(max_adv, 3), sig_id)
                        )
                    labelled += 1
                except Exception as e:
